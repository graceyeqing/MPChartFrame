...
import com.icaomei.mpchartlib.buffer.ScatterBuffer;
import com.icaomei.mpchartlib.charts.ScatterChart.ScatterShape;
import com.icaomei.mpchartlib.data.Entry;
import com.icaomei.mpchartlib.data.ScatterData;
import com.icaomei.mpchartlib.data.ScatterDataSet;
        ...
        switch (shape) {
            ...
            case CIRCLE:
                ...
                for (int i = 0; i < buffer.size(); i += 2) {
                    ...
                    mRenderPaint.setColor(dataSet.getColor(i / 2));
                    mRenderPaint.setStyle(Style.STROKE);//设置空心
        ...
        if (mChart.getScatterData().getYValCount() < mChart.getMaxVisibleCount()
            ...
            for (int i = 0; i < mChart.getScatterData().getDataSetCount(); i++) {
                ...
                for (int j = 0; j < positions.length * mAnimator.getPhaseX(); j += 2) {
                    ...
                    mValuePaint.setColor(Color.rgb(63, 63, 63));// ���û�ɫ
                    mValuePaint.setStyle(Style.FILL);//��������  
                    c.drawRect(positions[j]-50, positions[j + 1] - shapeSize-60, 
                    		positions[j]+50, positions[j + 1] - shapeSize-20, mValuePaint);// ������  
                    
                    mValuePaint.setColor(Color.rgb(255, 255, 255));// ���ð�ɫ
                    mValuePaint.setTextSize(Utils.convertDpToPixel(10f));
                    c.drawText(dataSet.getValueFormatter().getFormattedValue(val), positions[j],
                            positions[j + 1] - shapeSize-35,
                            mValuePaint);
                }
            }
        }
